...
// NOTE most of this unicode shenanigans is just a straight port given to me by an LLM, some of this stuff is so rarely used that I'll validate as the need comes
...
func IsIdentifierStart(code rune, astral bool) bool {
	...
	if code <= 0xffff {
		if code >= 0xaa {
			match, _ := regexp.Match(strings.Join([]string{"[", NonAsciiIdentifierASCIIStartChars, "]"}, ""), []byte{byte(code)}) // bit sketchy here, don't know if I have the patience to make this work? :/
			return match
		}
	}
	if !astral {
...
func IsInAstralSet(code rune, set []rune) bool {
	pos := 0x10000
	for i := 0; i < len(set); i = i + 2 {
		pos = pos + int(set[i])
		if pos > int(code) {
			return false
		}
...
func IsIdentifierChar(code int32, astral bool) bool {
	...
	if code < 0xffff {
		match, _ := regexp.Match(strings.Join([]string{"[", NonAsciiIdentifierASCIIStartChars, NonAsciiIdentifierAsciiChars, "]"}, ""), []byte{byte(code)})
		return match
	}
...
func hasOwn(obj map[string]interface{}, propName string) bool {
	_, exists := obj[propName]
	return exists
}
...
func WordsRegexp(words string) *regexp.Regexp {
	RegexpCache.RLock()
	if re, exists := RegexpCache.cache[words]; exists {
		RegexpCache.RUnlock()
		return re
	}
	RegexpCache.RUnlock()

	pattern := "^(?:" + strings.ReplaceAll(words, " ", "|") + ")$"
	re, err := regexp.Compile(pattern)
	if err != nil {
		panic("Invalid regexp pattern: " + pattern)
	}

	RegexpCache.Lock()
	RegexpCache.cache[words] = re
	RegexpCache.Unlock()

	return re
}

// kinda iffy with this one also :/ ...
func CodePointToString(code rune) string {
	if code <= 0xFFFF {
		return string(code)
	}
	code -= 0x10000
	high := (code >> 10) + 0xD800
	low := (code & 1023) + 0xDC00

	utf16Units := []uint16{uint16(high), uint16(low)}

	runes := utf16.Decode(utf16Units)
	return string(runes)
}

var LoneSurrogate = regexp.MustCompile(`[\x{0800}-\x{DFFF}]`)

type NonBinary struct {
	GeneralCategory  *regexp.Regexp
	Script           *regexp.Regexp
	ScriptExtensions *regexp.Regexp
	Gc               *regexp.Regexp // Alias for General_Category
	Sc               *regexp.Regexp // Alias for Script
	Scx              *regexp.Regexp // Alias for Script_Extensions
}

// Data holds Unicode properties for an ECMAScript version.
type Data struct {
	Binary          *regexp.Regexp
	BinaryOfStrings *regexp.Regexp
	NonBinary       NonBinary
}

// UnicodeData maps ECMAScript versions to their Unicode data.
var UnicodeData = make(map[int]Data)

// Unicode property definitions.
...
// unicodeBinaryPropertiesOfStrings maps ECMAScript versions to binary properties of strings.
var unicodeBinaryPropertiesOfStrings = map[int]string{
	9:  "",
	10: "",
	11: "",
	12: "",
...
// unicodeScriptValues maps ECMAScript versions to script values.
var unicodeScriptValues = map[int]string{
	9:  ecma9ScriptValues,
...
// buildUnicodeData constructs Unicode data for a given ECMAScript version.
func buildUnicodeData(ecmaVersion int) {
	d := Data{
		Binary:          WordsRegexp(strings.Join([]string{unicodeBinaryProperties[ecmaVersion], unicodeGeneralCategoryValues}, " ")),
		BinaryOfStrings: WordsRegexp(unicodeBinaryPropertiesOfStrings[ecmaVersion]),
		NonBinary: NonBinary{
			GeneralCategory: WordsRegexp(unicodeGeneralCategoryValues),
			Script:          WordsRegexp(unicodeScriptValues[ecmaVersion]),
		},
	}
	d.NonBinary.ScriptExtensions = d.NonBinary.Script
	d.NonBinary.Gc = d.NonBinary.GeneralCategory
...
func nextLineBreak(code []byte, from, end int) int {
	for i := from; i < end; i++ {
		next := code[i]

		if isNewLine(rune(next)) {
			if i < end-1 && next == 13 && code[i+1] == 10 {
				return i + 2
			} else {
				return i + 1
			}
