...
1. [Parse](runtime/parser/parser.go) javascript file into an [AST](https://en.wikipedia.org/wiki/Abstract_syntax_tree), (uses a [acorn](https://github.com/acornjs/acorn) port to go.)
...
6. VM picks up our function and begins executing
   - during execution, if a function gets called more than the threshold for "hot path", and the [JIT compiler](https://github.com/JeremiasRy/go_js/blob/master/runtime/jit/compiler.go) has translation steps for the bytecode used in the function, the VM requests a JIT compiled version of the function and executes it
7. If our VM is not executing and eventloop doesn't have any work, we have finished our program

```mermaid
flowchart TD
    A[Parser] -->|AST| B(Compile)
...
After the test run few benchmark scripts are also ran against nodeJS to compare the performance (essentially comparing against V8). Below is one example run summarized (Test suite was ran on my local machine)
