...
4. [Execute](runtime/vm/vm.go) the generated bytecode
   - if a function gets called more than the threshold for "hot path", and the [JIT compiler](https://github.com/JeremiasRy/go_js/blob/master/runtime/jit/compiler.go) has translation steps for the bytecode used in the function, execute JIT compilation and use the optimized machine code.
