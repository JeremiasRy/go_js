...
<script lang="ts">
    ...
    let {node, setHighlight, highlight}: {node: AstNode, setHighlight: (highlight: HighlightType) => void, highlight: HighlightType} = $props()
    ...
    const propIsAstNode = (prop: unknown): boolean => {
        if (typeof prop === "object" && prop !== null) {
            return "id" in prop && "type" in prop && "start" in prop && "end" in prop
        }
    ...
    const onMouseEnter = (e: Event) => {
        e.stopPropagation()
        setHighlight({ast_ids: [...(node.ast_train) ?? [], node.id], from: node.start, to: node.end})
    }
    ...
    $effect(() => {
        if (highlight.ast_ids.length === 1 && highlight.from === 0 && highlight.to === 0 && node.start !== 0 && node.end !== 0) {
            setHighlight({ast_ids: highlight.ast_ids, from: node.start, to: node.end})
        }
    })
                    ...
                    {#if Array.isArray(value)}
                        ...
                        {#each value as possibleNode}
                            {#if propIsAstNode(possibleNode as object)}
                                <AstTree node={possibleNode as AstNode} highlight={highlight} setHighlight={setHighlight}/>
                            {/if}
                        {/each}
                    {:else}
                        {#if propIsAstNode(value as object)}
                            <i>{camelToCapital(key)}</i> <AstTree node={value as AstNode} highlight={highlight} setHighlight={setHighlight}/>
